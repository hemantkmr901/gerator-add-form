...
import { MyTextInput } from "./EditDeviceStep1";
import { updateAppDataEditDevice, clearAppDataEditDevice } from "./AppDataEditDeviceSlice";
import { useUpdateDeviceMutation } from "../../api/AddDeviceAPI";
...
const EditDeviceStep3 = () => {
  ...
  const { cropedImageFile, setCropedImageFile, selectedMultiImageFile, setSelectedMultiImageFile, selectedDocumentFile, setSelectedDocumentFile, selectedImageFile, setSelectedImageFile } = useContext(EditDeviceFormContext);

  // const { selectedDocumentFile, setSelectedDocumentFile, selectedMultiImageFile, selectedImageFile, setSelectedImageFile, setSelectedMultiImageFile } = useContext(EditDeviceFormContext);
  const fileDocRef = useRef(null);
  const [fileDoceLinkRemover, setFileDocLinkRemover] = useState(false);


  const prev = () => dispatch(updateAppDataEditDevice({ case: "CURRENTSTEP", value: (appData.currentStep - 1) }));
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        ...
        //Setting to default value of hooks
        setCropedImageFile("")
        setSelectedMultiImageFile("")
        setSelectedDocumentFile("")

        //This code will be used for making request from RTK Query
        // addDevice(multiPartData);
        updateDevice({ id: values.id, body: multiPartData });
...
export default EditDeviceStep3;
